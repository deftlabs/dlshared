...
func TestLevels(test *testing.T) {
	...
	if levelType != "dlshared.Level" {
		test.Errorf("Bad Level type. Expected: `dlshared.Level` Received: `%v`", levelType)
	}
}
...
func TestStacktrace(test *testing.T) {
	...
	if match, _ := regexp.MatchString("^at dev/golang-shared/logger_test.go:\\d+", stacktrace[0]); !match {
...
func TestStackError(test *testing.T) {
	...
	if match, _ := regexp.MatchString("logger_test.go:\\d+", str); match == false {
		test.Errorf("Expected to see output for `logger_test.go`. Received:\n%v", str)
	}

	match, err := regexp.MatchString("logger.go:\\d+", str)
	if err != nil {
		test.Errorf("Error matching: %v", err)
	}

	if match == true {
		test.Errorf("The stacktrace should have no output from dlshared/logger.go. Received:\n%v", str)
	}
}
