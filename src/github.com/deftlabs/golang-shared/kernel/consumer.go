...
import (
	...
	"github.com/deftlabs/golang-shared/util"
)
...
func NewConsumer(	name string,
					...
					logger slogger.Logger) *Consumer {
...
func (self *Consumer) Stop() error {
	...
	if self.maxWaitOnStopInMs > 0 {
		start := deftlabsutil.CurrentTimeInMillis()
		...
		for {
			if len(self.processorChannel) == 0 {
		 	...
		 	if deftlabsutil.CurrentTimeInMillis() - start >= self.maxWaitOnStopInMs {
				break
			}
